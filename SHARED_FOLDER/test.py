...
# archive_path = "\\\\192.168.0.12\\homes\\Jason\\test_video"
...
all_file_list = glob.glob(archive_path +"\\*.avi")
file_size_list = [ os.path.getsize(file_path) for file_path in all_file_list ]
duration_list = list()
...
zipped_lists = list( zip(all_file_list, file_size_list, duration_list,) )
# print(zipped_lists)
...
print( '사용가능 파일 갯수= {}, 비율= {}%'.format(
    len(workable_list), int(100*len(workable_list)/len(all_file_list))) )
